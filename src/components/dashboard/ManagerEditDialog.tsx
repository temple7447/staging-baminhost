...
interface ManagerEditDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  manager: Manager | null;
  estates: Estate[];
  onSave: (id: string, data: UpdateManagerData) => Promise<void>;
  isLoading?: boolean;
}
...
export const ManagerEditDialog: React.FC<ManagerEditDialogProps> = ({
  open,
  onOpenChange,
  manager,
  estates,
  ...
  useEffect(() => {
    if (manager) {
      setFormData({
        ...
        email: manager.email || '',
        phone: manager.phone || '',
        estateIds: manager.assignedEstates?.map((e) => e._id) || [],
      });
    }
  }, [manager, open]);
  ...
  const handleSubmit = async () => {
    if (!manager) return;
    try {
      await onSave(manager._id, formData);
      onOpenChange(false);
    } catch (error) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>Update manager information and assigned estates</DialogDescription>
        </DialogHeader>
